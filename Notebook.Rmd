...
#Hier bearbeiten wir die variablen mit Lag Check this and continue from here
missing_lagged_var <- zusamengefuehrt %>%
  filter(is.na(lagged_var_1))

# Step 2: Fit a regression model using the lagged value of lagged_var_1 as a predictor
model_lagged_var_1 <- lm(lagged_var_1 ~ lag(lagged_var_1, 1), data = zusamengefuehrt)

# Step 3: Predict missing values using the fitted model
missing_lagged_var$lagged_var_1 <- predict(model_lagged_var_1, newdata = missing_lagged_var)

# Merge the data back into the original data frame
zusamengefuehrt <- bind_rows(zusamengefuehrt %>% filter(!is.na(lagged_var_1)), missing_lagged_var)

# Arrange the data frame by 'Datum' and 'Warengruppe'
zusamengefuehrt <- zusamengefuehrt %>%
